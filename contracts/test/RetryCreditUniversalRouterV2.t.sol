...
import {
    AttestcoinRetryCreditUniversalRouterVerifierV2
...
contract RetryCreditUniversalRouterV2Test is Test {
    ...
    uint256 private constant BENEFICIARY_KEY = 0xBEEF;
    ...
    RetryCreditUniversalRouterPredicateV2 private predicate;
    MockNativeQueryVerifier private nativeVerifier;
    AttestcoinRetryCreditUniversalRouterVerifierV2 private batchVerifier;
    MockChainInfo private chainInfo;
    RetryCreditUniversalRouterPoolV2 private servicePool;
    ...
    function setUp() external {
        ...
        predicate = new RetryCreditUniversalRouterPredicateV2();
        nativeVerifier = new MockNativeQueryVerifier();
        batchVerifier = new AttestcoinRetryCreditUniversalRouterVerifierV2(
        ...
        servicePool = new RetryCreditUniversalRouterPoolV2(batchVerifier, address(chainInfo));
    ...
    function testFundedPoolActivatesReleasesOnceAndConsumesBatchIdentity() external {
        ...
        RetryCreditUniversalRouterPoolV2.ServiceCredit memory credit = servicePool.getServiceCredit(serviceCreditNumber);
        ...
        bytes32 actionKey =
            keccak256(abi.encode(SOURCE_CHAIN_KEY, SOURCE_CHAIN_ID, ROUTER, trader, beneficiary, ACTION_ID));
        ...
        vm.expectRevert(RetryCreditUniversalRouterPoolV2.AlreadyResolved.selector);
        vm.prank(prover);
        servicePool.releaseCredit(serviceCreditNumber, proof);
    }

    function testReleaseProofMustMatchCommittedSourceTransactions() external {
    ...
    function testSponsorCommitsRestartSafeSourceTransactionsBeforeBroadcast() external {
        (uint256 serviceCreditNumber,) = _createAndActivateServiceCredit();
        bytes memory failed = hex"02aabb";
        bytes memory successful = hex"02ccdd";

        vm.prank(sponsor);
        servicePool.commitSourceTransactions(serviceCreditNumber, failed, successful);
        RetryCreditUniversalRouterPoolV2.SourceTransactions memory stored =
            servicePool.getSourceTransactions(serviceCreditNumber);
        assertEq(keccak256(stored.failed), keccak256(failed));
        assertEq(keccak256(stored.successful), keccak256(successful));

        vm.expectRevert(RetryCreditUniversalRouterPoolV2.SourceTransactionsAlreadyCommitted.selector);
        vm.prank(sponsor);
        servicePool.commitSourceTransactions(serviceCreditNumber, failed, successful);

        (uint256 secondCredit,) = _createAndActivateServiceCredit();
        vm.expectRevert(RetryCreditUniversalRouterPoolV2.NotSponsor.selector);
        vm.prank(prover);
        servicePool.commitSourceTransactions(secondCredit, failed, successful);
    }

    function testFundedPoolPolicyIsDerivedAfterDraftAndBoundIntoRoute() external {
        RetryCreditUniversalRouterPredicateV2.Rule memory terms = _terms();
        vm.prank(sponsor);
        uint256 serviceCreditNumber =
            servicePool.createServiceCredit{value: CREDIT_AMOUNT}(terms, uint64(block.timestamp + 7 days));
        RetryCreditUniversalRouterPoolV2.ServiceCredit memory draft = servicePool.getServiceCredit(serviceCreditNumber);

        vm.expectRevert(RetryCreditUniversalRouterPoolV2.ActivationNotReady.selector);
        vm.prank(sponsor);
        ...
        bytes32 expected = keccak256(
            abi.encode(
                "RETRYCREDIT_UNISWAP_SERVICE_CREDIT_V2",
                block.chainid,
                address(servicePool),
                serviceCreditNumber,
                sponsor,
    ...
    function testFundedPoolRejectsRetroactiveWindowAndUnsafeRefund() external {
        ...
        vm.expectRevert(RetryCreditUniversalRouterPoolV2.SourceWindowNotAttested.selector);
        vm.prank(sponsor);
        servicePool.createServiceCredit{value: CREDIT_AMOUNT}(terms, uint64(block.timestamp + 7 days));
    ...
    function testValidOfficialRouterRetryFitsOneNativeAttestcoinBatch() external {
        AttestcoinRetryCreditUniversalRouterVerifierV2.BatchProof memory proof = _validProof();
        nativeVerifier.setTransactionIndexForRoot(proof.merkleProofs[0].root, 7);
        nativeVerifier.setTransactionIndexForRoot(proof.merkleProofs[1].root, 11);

        (address returnedBeneficiary, bytes32 failureQueryId, bytes32 successQueryId, bytes32 pairId) =
            batchVerifier.verifyRelease(proof, _rule());

        assertEq(returnedBeneficiary, beneficiary);
    ...
    function testDomainTypedDataAndStableIntentMatchUniversalRouterV2() external view {
        ...
        bytes32 expectedIntent = keccak256(
            abi.encode("RETRYCREDIT_UNISWAP_V2", POLICY_ID, ACTION_ID, trader, beneficiary, WETH, USDC, POOL, AMOUNT_IN)
        );
    ...
    function testTermsRequireExactOfficialSepoliaStackAndDistinctRoles() external {
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidRule.selector);
        predicate.validateRule(terms);

        terms = _rule();
        terms.beneficiary = trader;
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidRule.selector);
        predicate.validateRule(terms);
    ...
    function testBatchVerifierRejectsAnyNonSepoliaAttestcoinSourceIdentity() external {
        vm.expectRevert(AttestcoinRetryCreditUniversalRouterVerifierV2.InvalidConfiguration.selector);
        new AttestcoinRetryCreditUniversalRouterVerifierV2(
            predicate, address(nativeVerifier), uint64(3), SOURCE_CHAIN_ID
        );
    ...
    function testBothRoutesNeedIndependentCorrectSignaturesAndBoundSender() external {
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidRouteSignature.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidParticipant.selector
        );
    }
    ...
    function testRejectsWrongIntentRouteDataAndReplayBypassNonce() external {
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidIntent.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidRouteData.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidRouteNonce.selector
        );
    }
    ...
    function testRejectsWrongCommandsWrapAndNestedV3Route() external {
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidCommands.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidRouteInput.selector
        );
    ...
    function testRejectsNonCanonicalNestedInputEvenWhenDecodedValuesMatch() external {
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidRouteInput.selector);
        predicate.validate(
            _encodedAttempt(
    ...
    function testRetryMustActuallyRefreshNonceDeadlineAndStaleMinimum() external {
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidDeadlineRefresh.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.SettledValueTooLow.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            _successfulReceipt(),
            RetryCreditUniversalRouterPredicateV2.InvalidRouteRefresh.selector
        );
    }
    ...
    function testRejectsNonCanonicalOuterOffsetsTrailingBytesAndFailureLogs() external {
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidCalldata.selector);
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidReceiptSequence.selector);
        predicate.validate(failed, FAILURE_BLOCK, succeeded, SUCCESS_BLOCK, SOURCE_CHAIN_ID, _rule());
    }
    ...
    function testSuccessRequiresExactOfficialPoolSwapAndCircleTransfer() external {
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            success,
            RetryCreditUniversalRouterPredicateV2.RequiredPoolSwapMissing.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            success,
            RetryCreditUniversalRouterPredicateV2.RequiredSettlementTransferMissing.selector
        );
    ...
    function testRejectsDuplicatePoolOrSettlementEvidence() external {
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            success,
            RetryCreditUniversalRouterPredicateV2.DuplicatePoolSwap.selector
        );
        ...
        _expectFailure(
            ...
            _failedReceipt(),
            success,
            RetryCreditUniversalRouterPredicateV2.DuplicateSettlementTransfer.selector
        );
    }
    ...
    function testRejectsEnvelopeReceiptWindowAndNativeVerifierFailures() external {
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidParticipant.selector);
        predicate.validate(failed, FAILURE_BLOCK, succeeded, SUCCESS_BLOCK, SOURCE_CHAIN_ID, _rule());

        failed = _encodedAttempt(
            failedData, FAILURE_TRANSACTION_NONCE, trader, ROUTER, AMOUNT_IN + 1, MINIMUM_GAS_LIMIT, _failedReceipt()
        );
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidTransactionValue.selector);
        predicate.validate(failed, FAILURE_BLOCK, succeeded, SUCCESS_BLOCK, SOURCE_CHAIN_ID, _rule());
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.FailureGasExceeded.selector);
        predicate.validate(failed, FAILURE_BLOCK, succeeded, SUCCESS_BLOCK, SOURCE_CHAIN_ID, _rule());

        AttestcoinRetryCreditUniversalRouterVerifierV2.BatchProof memory proof = _validProof();
        nativeVerifier.setVerificationResult(false);
        vm.expectRevert(AttestcoinRetryCreditUniversalRouterVerifierV2.ProofVerificationFailed.selector);
        batchVerifier.verifyRelease(proof, _rule());
    }
    ...
    function testRejectsTargetGasChainStatusBlockGapAndTransactionNonceRegression() external {
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidTarget.selector);
        predicate.validate(failed, FAILURE_BLOCK, succeeded, SUCCESS_BLOCK, SOURCE_CHAIN_ID, _rule());

        failed = _encodedAttempt(
            failedData, FAILURE_TRANSACTION_NONCE, trader, ROUTER, AMOUNT_IN, MINIMUM_GAS_LIMIT - 1, _failedReceipt()
        );
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.AttemptGasLimitTooLow.selector);
        predicate.validate(failed, FAILURE_BLOCK, succeeded, SUCCESS_BLOCK, SOURCE_CHAIN_ID, _rule());

        failed = _encodedAttempt(
            failedData, FAILURE_TRANSACTION_NONCE, trader, ROUTER, AMOUNT_IN, MINIMUM_GAS_LIMIT, _failedReceipt()
        );
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidSourceChain.selector);
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidBlockGap.selector);
        ...
        vm.expectRevert(RetryCreditUniversalRouterPredicateV2.InvalidRouteRefresh.selector);
        predicate.validate(failed, FAILURE_BLOCK, succeeded, SUCCESS_BLOCK, SOURCE_CHAIN_ID, _rule());
    }
    ...
    function _rule() private view returns (RetryCreditUniversalRouterPredicateV2.Rule memory rule) {
        return _ruleWithPolicy(POLICY_ID);
    }

    function _terms() private view returns (RetryCreditUniversalRouterPredicateV2.Rule memory rule) {
    ...
    function _ruleWithPolicy(bytes32 policyId)
        private
        view
        returns (RetryCreditUniversalRouterPredicateV2.Rule memory rule)
    {
        rule = RetryCreditUniversalRouterPredicateV2.Rule({
    ...
    function _routeForRule(bool successful, RetryCreditUniversalRouterPredicateV2.Rule memory rule)
        private
        view
        returns (RouteSpec memory route)
    {
        ...
        route.outputRecipient = rule.beneficiary;
    ...
    function _validProof()
        private
        view
        returns (AttestcoinRetryCreditUniversalRouterVerifierV2.BatchProof memory proof)
    {
        return _validProofForRule(_rule());
    }

    function _validProofForRule(RetryCreditUniversalRouterPredicateV2.Rule memory rule)
        private
        view
        returns (AttestcoinRetryCreditUniversalRouterVerifierV2.BatchProof memory proof)
    {
    ...
    function _successfulReceipt() private view returns (ReceiptSpec memory receipt) {
        ...
        receipt.swapRecipient = beneficiary;
        ...
        receipt.transferTo = beneficiary;
