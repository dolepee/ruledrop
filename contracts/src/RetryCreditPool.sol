...
import {AttestcoinRetryCreditVerifier} from "./AttestcoinRetryCreditVerifier.sol";
import {RetryCreditPredicateV2} from "./RetryCreditPredicateV2.sol";
import {IChainInfo} from "./interfaces/IChainInfo.sol";

/// @notice Holds one pre-funded service credit until Attestcoin proves an authorized failed-to-settled retry.
contract RetryCreditPool is ReentrancyGuard {
    ...
    AttestcoinRetryCreditVerifier public immutable retryVerifier;
    ...
    mapping(uint256 serviceCreditNumber => RetryCreditPredicateV2.Rule) private rules;
    ...
    constructor(AttestcoinRetryCreditVerifier retryVerifier_, address chainInfoOverride) {
    ...
    function createServiceCredit(RetryCreditPredicateV2.Rule calldata terms, uint64 refundAfter)
        external
        payable
        returns (uint256 serviceCreditNumber)
    {
        ...
        emit ServiceCreditDraftCreated(
            serviceCreditNumber, msg.sender, terms.beneficiary, msg.value, refundAfter, block.number, termsHash
        );
    }

    /// @notice Derives a stable, unpredictable policyId from the mined draft block.
    ...
    function releaseCredit(uint256 serviceCreditNumber, AttestcoinRetryCreditVerifier.BatchProof calldata proof)
        external
        nonReentrant
    {
        ServiceCredit storage credit = _serviceCredit(serviceCreditNumber);
    ...
    function getRule(uint256 serviceCreditNumber) external view returns (RetryCreditPredicateV2.Rule memory) {
    ...
    function _actionKey(RetryCreditPredicateV2.Rule storage rule) private view returns (bytes32) {
        return keccak256(abi.encode(sourceChainKey, sourceChainId, rule.target, rule.beneficiary, rule.actionId));
    }
