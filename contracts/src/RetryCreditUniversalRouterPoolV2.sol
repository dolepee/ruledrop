...
contract RetryCreditUniversalRouterPoolV2 is ReentrancyGuard {
    ...
    struct SourceTransactions {
        bytes failed;
        bytes successful;
    }

    AttestcoinRetryCreditUniversalRouterVerifierV2 public immutable retryVerifier;
    RetryCreditUniversalRouterPredicateV2 public immutable predicate;
    ...
    mapping(uint256 serviceCreditNumber => RetryCreditUniversalRouterPredicateV2.Rule) private rules;
    mapping(uint256 serviceCreditNumber => SourceTransactions) private sourceTransactions;
    ...
    event SourceTransactionsCommitted(
        uint256 indexed serviceCreditNumber, bytes32 indexed failureHash, bytes32 indexed successHash
    );
    ...
    error InvalidSourceTransactions();
    error NotActivated();
    error NotSponsor();
    error RefundClosed();
    error ReleaseClosed();
    error Replay();
    error SourceTransactionsAlreadyCommitted();
    error SourceWindowNotAttested();
    error TransferFailed();

    constructor(AttestcoinRetryCreditUniversalRouterVerifierV2 retryVerifier_, address chainInfoOverride) {
    ...
    function createServiceCredit(RetryCreditUniversalRouterPredicateV2.Rule calldata terms, uint64 refundAfter)
        external
        payable
    ...
    /// @notice Persists both signed source transactions before either is broadcast so execution is restart-safe.
    function commitSourceTransactions(uint256 serviceCreditNumber, bytes calldata failed, bytes calldata successful)
        external
    {
        ServiceCredit storage credit = _serviceCredit(serviceCreditNumber);
        if (msg.sender != credit.sponsor) revert NotSponsor();
        ...
        if (sourceTransactions[serviceCreditNumber].failed.length != 0) revert SourceTransactionsAlreadyCommitted();
        if (failed.length == 0 || successful.length == 0 || failed.length > 4096 || successful.length > 4096) {
            revert InvalidSourceTransactions();
        }
        bytes32 failureHash = keccak256(failed);
        bytes32 successHash = keccak256(successful);
        if (failureHash == successHash) revert InvalidSourceTransactions();
        sourceTransactions[serviceCreditNumber] = SourceTransactions({failed: failed, successful: successful});
        emit SourceTransactionsCommitted(serviceCreditNumber, failureHash, successHash);
    }

    function releaseCredit(
        uint256 serviceCreditNumber,
        AttestcoinRetryCreditUniversalRouterVerifierV2.BatchProof calldata proof
        ...
        emit CreditReleased(
            ...
            beneficiary,
            rules[serviceCreditNumber].trader,
            creditAmount,
            failureQueryId,
            successQueryId,
            pairId,
    ...
    function getRule(uint256 serviceCreditNumber)
        external
        view
        returns (RetryCreditUniversalRouterPredicateV2.Rule memory)
    {
        _serviceCredit(serviceCreditNumber);
        return rules[serviceCreditNumber];
    }

    function getSourceTransactions(uint256 serviceCreditNumber) external view returns (SourceTransactions memory) {
        _serviceCredit(serviceCreditNumber);
        return sourceTransactions[serviceCreditNumber];
    }
    ...
    function _actionKey(RetryCreditUniversalRouterPredicateV2.Rule storage rule) private view returns (bytes32) {
        return
            keccak256(
                abi.encode(sourceChainKey, sourceChainId, rule.router, rule.trader, rule.beneficiary, rule.actionId)
            );
    }
