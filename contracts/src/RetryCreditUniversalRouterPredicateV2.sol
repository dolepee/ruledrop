...
contract RetryCreditUniversalRouterPredicateV2 {
    ...
    function _validateRule(Rule calldata rule, bool requirePolicyId) private pure {
        if (
            rule.routeSigner == address(0) || rule.trader == address(0) || rule.beneficiary == address(0)
                || rule.routeSigner == rule.trader || rule.beneficiary == rule.trader
        ...
        returns (uint256 amountOutMinimum)
    {
        ...
        if (
            keccak256(inputs[1])
                ...
                || recipient != rule.beneficiary || amountIn != rule.amountIn || amountOutMinimum == 0 || payerIsUser
        ...
        for (uint256 i; i < logs.length; ++i) {
            ...
            if (
                logs[i].topics.length != 3 || logs[i].data.length != 160
                    ...
                    || logs[i].topics[2] != bytes32(uint256(uint160(rule.beneficiary)))
